...
function StreamingRoom(): JSX.Element {
  const [messages, setMessages] = useState<{ text: string; username: string; timestamp: Date }[]>(
    []
  );
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (inputRef.current && inputRef.current.value.trim() !== "") {
      setMessages([
        ...messages,
        { username: '', text: inputRef.current.value.trim(), timestamp: new Date() },
      ]);
